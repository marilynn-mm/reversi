...
# counting / checking winning condition |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    def winFor(self):
        """
        When the board is full, this method counts each player's pieces and
        determines a winner
        """
        ...
        if self.isFull(): 
            ...
            # gives the user the option to restart game
            print ()
...
# Smart AI |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    def listOfMaxMove(self, ox):
        """from list of move
        find move/moves with highest 'score' or number of potential flips 
        """
    ...
    def listOfEmptyCells(self):
        """
        find list of empty cells on board
        """
        listOfEmptyCells = []
...
# Reversi Move |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    def smartAIMove(self, ox):
        """
        Ai places the move with the max number of flips. If multiple cells have the
        same number of flips, it randomizes where to play
        """
...
# host game |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    ...
    def hostGame(self):
        ...
        # while the function switches ox between 'X' and 'O', we've created a variable
        # ox2 regardless as it will be helpful in the self.listOfMoves() call
        ox2 = 'O'
        ...
        while True:
            ...
            # If the board is not full, but neither player can make a move
            if not self.isFull():
                if self.listOfMoves(ox) == [] and self.listOfMoves(ox2) == []:
                    random_empty_cell = random.choice(self.listOfEmptyCells())
                    row, col = random_empty_cell
                    self.addMove(row, col, ox2)

            # Once the board is full, break from the loop and process who won
            if self.isFull():
                break 
    ...
    def host2AIGame(self):
        """
        host a game of reversi between two smart AI
        alternate turns between 'X' (who will always go first) and 'O' (who will always go second)
        ask the user (with the input function) to select a column number for each move
        """
        b.setUp()
